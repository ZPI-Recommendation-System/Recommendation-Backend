...
describe('LaptopsController', () => {
  ...
  describe('root', () => {
      ...
      expect(
        await laptopController
          .getLaptops(
            {
              ids: [
                '003bb070-d6d8-4c9d-a7b4-26f515c1ad4c',
                '003d0482-5807-4589-808e-9674490aba58',
              ],
              query: [],
            },
            new SortingDto(),
    ...
    it('Benchmark stats', async () => {
      expect(await laptopController.getBenchmarkStats()).toEqual([
        { benchmark_type: 'CPU', max: 87.9, min: 40.4 },
        { benchmark_type: 'GPU', max: 88.6, min: 2.22 },
      ]);
    });
  });
});
